# Based on https://github.com/OSGeo/gdal/tree/v3.5.3/docker
...
RUN . /buildscripts/bh-set-envvars.sh \
    ...
    && DEBIAN_FRONTEND=noninteractive apt-get install -y --fix-missing --no-install-recommends \
       ...
       swig ant openjdk-"$JAVA_VERSION"-jdk${APT_ARCH_SUFFIX} \
...
RUN . /buildscripts/bh-set-envvars.sh \
    ...
    && mkdir build \
    && cd build \
    # Comment the loading of `gdalalljni` because it will be taken care. Otherwise we will see warning in console
    && sed -i 's/System.loadLibrary(\"gdalalljni\");//g' ../swig/include/java/gdal_java.i \
    && CFLAGS='-DPROJ_RENAME_SYMBOLS -O2' CXXFLAGS='-DPROJ_RENAME_SYMBOLS -DPROJ_INTERNAL_CPP_NAMESPACE -O2' \
       cmake .. \
        -DCMAKE_INSTALL_PREFIX=/usr \
